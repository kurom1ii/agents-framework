...
This document provides comprehensive code examples for various CrewAI use cases.

## Table of Contents

1. [Basic Crew Setup](#basic-crew-setup)
2. [YAML-Based Configuration](#yaml-based-configuration)
3. [Multi-Agent Workflows](#multi-agent-workflows)
4. [Custom Tools](#custom-tools)
5. [Flows Examples](#flows-examples)
6. [Memory and Knowledge](#memory-and-knowledge)
...
Based on the official CrewAI examples repository.
...
Based on the official CrewAI examples.
...
# Second interaction - will remember context
...
# Or crew-level knowledge (shared by all agents)
crew = Crew(
    agents=[support_agent],
    tasks=[support_task],
    knowledge_sources=[company_info]  # All agents can access
)
```
...
crew = Crew(
    agents=[agent],
    tasks=[task],
    max_rpm=60,  # Crew-level rate limit
...
# Strategic planner uses powerful model
planner = Agent(
...
# Data formatter uses cheaper model
formatter = Agent(
    role="Data Formatter",
    goal="Format data consistently",
    backstory="Data processing expert",
    llm=simple_llm,
    function_calling_llm=simple_llm  # Also use cheap model for tools
)
```
